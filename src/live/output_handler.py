...
class OutputController:
    ...
    def __init__(self, 
        ...
        """Initialize output controller with display settings."""
    ...
    def _initialize_display(self):
        ...
        logger.info("Pygame display başlatılıyor...")
        try:
            ...
            logger.info(f"Pygame display başarıyla başlatıldı. Boyut: {self.window_size}, Font boyutu: {self.base_font_size}")
        except pygame.error as e:
            logger.error(f"Pygame display başlatılırken hata: {e}")
            self.screen = None
        except Exception as e:
            logger.error(f"Pygame display başlatılırken genel bir hata oluştu: {e}")
            self.screen = None

    def display_frame(self, frame_bgr, display_info):
        """
        Displays the video frame and emotion information on the Pygame window.
        frame_bgr: OpenCV BGR video frame. If None, only info area is shown.
        display_info: Dictionary containing information to display.
        """
        ...
        if frame_bgr is not None:
            try:
                ...
                logger.error(f"Video karesi Pygame\'e dönüştürülürken veya çizilirken hata: {e}", exc_info=False)
        ...
        for key, value in info_lines_content.items():
            try:
                ...
                y_offset += self.line_height
                 ...
                 logger.error(f"Metin çizilirken hata ({key}): {e}")
                 y_offset += self.line_height
            ...
            if isinstance(probs_dict_or_list, dict):
                ...
                if len(probs_dict_or_list) == len(target_labels):
                    ...
                    items_to_render = sorted(temp_dict.items(), key=lambda item: item[1], reverse=True)
                     ...
                     logger.warning(f"'{title}' için olasılık listesi ve etiket listesi uzunlukları eşleşmiyor.")
        ...
        if self.show_integrated_probs:
            ...
            if target_emotions_for_display:
                ...
                logger.warning("Integrated probs gösterimi için config.TARGET_EMOTIONS bulunamadı.")
            
        pygame.display.flip()

    def handle_events(self):
        """
        Handles Pygame events (e.g., window close, keyboard input).
        Returns:
            bool: False if the main loop should exit (e.g., quit request).
        """
        if not self.screen: return True 

        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                logger.info("Pygame penceresi kapatma isteği alındı.")
                return False
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_q or event.key == pygame.K_ESCAPE:
                    logger.info("Kullanıcı 'q' veya 'ESC' tuşuna bastı. Çıkış yapılıyor.")
                    return False
                if event.key == pygame.K_f:
                    ...
                    if self.enable_fullscreen:
                        ...
                        self.screen = pygame.display.set_mode(self.window_size, pygame.RESIZABLE | pygame.DOUBLEBUF)
                    logger.info(f"Tam ekran modu {'aktif' if self.enable_fullscreen else 'devre dışı'}.")
            ...
            if event.type == pygame.VIDEORESIZE: 
                ...
                try:
                    ...
                    self.pygame_font = pygame.font.Font(None, self.base_font_size)
                logger.info(f"Pygame penceresi yeniden boyutlandırıldı: {self.window_size}, Yeni font: {self.base_font_size}")

        return True

    def quit(self):
        """Quits pygame and releases display resources."""
        logger.info("Pygame display ve kaynakları serbest bırakılıyor...")
        try:
            if pygame.display.get_init():
                pygame.display.quit()
                logger.info("Pygame display kapatıldı.")
            
            if pygame.get_init():
                 pygame.quit()
                 logger.info("Genel pygame.quit() çağrıldı.")

        except Exception as e:
            logger.error(f"Pygame kapatılırken hata: {e}", exc_info=True)
